...
function RoomDetailPage() {
    ...
    const onFinish = async (values: any) => {
        ...
        try {
            ...
            const response = await fetch(`http://localhost:8001/room/create-issue/${id}`, {
                method: 'POST',
                headers: {
            ...
            if (response.ok) {
                ...
                console.log('Failed to post comment: Access Forbidden');
...
{roomDetails?.users?.some(user => user.email === userMail) ? 
                                    <p className='text-green-600 font-thin italic text-lg ml-5'>(Current renting)</p> : <p></p>}
                                </div>
                                <div className='flex space-x-10'>
                            ...
                            <div className='mt-5 p-10 rounded-lg border-solid border border-slate-400 justify-between'>
                                ...
                                <div className='flex justify-between'>
                                    <div>
                                        ...
                                        <p className='font-semibold text-[#273895] text-3xl'>{`${formatter(roomDetails?.price)}/tháng`}</p>
                                    </div>
                                    <div>
                            ...
                            {roomDetails?.users?.some(user => user.email === userMail) && (
                                <div className=' flex space-x-8 mt-5 p-10 rounded-lg border-solid border border-slate-400'>
                                    ...
                                    <p className='text-xl italic'>{`${roomDetails?.contracts[0].startDate} - ${roomDetails?.contracts[0].endDate}`}</p>
                        ...
                        <div className='flex-col flex-1 space-y-6'>
                            ...
                            <div className="flex-col p-10 space-y-4 rounded-lg border-solid border border-slate-400">
                                ...
                                <Form onFinish={onFinish} layout="inline">
                                    <Form.Item className="w-full" name="content" rules={[{ required: true, message: '' }]}>
